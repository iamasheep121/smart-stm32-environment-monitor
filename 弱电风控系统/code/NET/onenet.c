...
#define PROID			"o5H719J5gg"

#define ACCESS_KEY		"v5UtP5P7v3/Ll42R9yehYQc/s0WdyR0Jza+F03f99f8="
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	short result = 0;
	cJSON *json, *params_json, *led_json, *alarm_json, *fanspeed_json, *fan_state_json;
//	char *dataPtr = NULL;
//	char numBuf[10];
//	int num = 0;
	 uint8_t  fan_speed;//
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if(json == NULL)//对解析失败处理
				{
					UsartPrintf(USART_DEBUG, "JSON Parse Failed\r\n");
					break;
				}			
				...
				fan_state_json = cJSON_GetObjectItem(params_json,"fan");
				fanspeed_json = cJSON_GetObjectItem(params_json,"Anemo");//寻找风机速度标识符
...
//							alarm_flag = 1;
							UsartPrintf(USART_DEBUG, "开启警报\r\n");	
							Buzzer_ON();						
					}
					else 
					{
//							alarm_flag = 0;
							UsartPrintf(USART_DEBUG, "关闭警报\r\n");		
							Buzzer_OFF();
					}
				}
				//这个控制电机速度
				if(fanspeed_json != NULL)
				{
					 fan_speed = fanspeed_json->valueint; 
					if(fan_speed <= 100)
                     {
//                           fan_flag = 1;
                           Motor_Speed(fan_speed);
                           UsartPrintf(USART_DEBUG, "风机速度设置为: %d\r\n", fan_speed);
                     }
                       else
                     {
                        UsartPrintf(USART_DEBUG, "风机速度范围错误: %d\r\n", fan_speed);
                     }								
				}
				//这个控制电机状态（开启和关闭）
				if(fan_state_json != NULL)
				{
						if(fan_state_json->type == cJSON_True) 
					{				
							UsartPrintf(USART_DEBUG, "风机启动\r\n");	
							Motor_Speed(PLATFORM_MOTOR_ENABLE_SPEED);						
					}
					else 
					{		
							UsartPrintf(USART_DEBUG, "风机关闭\r\n");		
							Motor_Speed(PLATFORM_MOTOR_DISABLE_SPEED);
					}
				}
